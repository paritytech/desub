...
impl SubstrateType {
	...
	pub fn from_scale_info_type(
		ty: &ScaleInfoType,
		registry: &PortableRegistry,
	) -> Result<SubstrateType, ConvertError> {
		let def = ty.type_def();
		match def {
			...
			ScaleInfoTypeDef::Variant(inner) => {
				...
				Ok(SubstrateType::Variant(VariantType { name, variants: variants_iter.collect::<Result<_, _>>()? }))
			}
			...
			ScaleInfoTypeDef::Tuple(inner) => {
				let fields_iter = inner.fields().iter().map(|ty| resolve_to_substrate_type(ty, registry));
				let fields = fields_iter.collect::<Result<_, _>>()?;
				Ok(SubstrateType::Tuple(TupleType { fields }))
			}
			ScaleInfoTypeDef::Primitive(inner) => Ok(SubstrateType::Primitive(inner.clone().into())),
...
fn composite_fields(
	...
	registry: &PortableRegistry,
) -> Result<CompositeTypeFields, ConvertError> {
	...
	match are_fields_named {
		...
		false => {
			let unnamed = named_fields.map(|f| f.map(|t| t.1)).collect::<Result<_, _>>()?;
...
fn resolve_to_substrate_type(
	ty: &<scale_info::form::PortableForm as scale_info::form::Form>::Type,
	registry: &PortableRegistry,
) -> Result<SubstrateType, ConvertError> {
...
) -> Result<&'a ScaleInfoType, ConvertError> {
