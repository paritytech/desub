...
use crate::regex;
use frame_metadata::v9::{self, RuntimeMetadataV9, StorageEntryModifier, StorageEntryType, StorageHasher};
use std::{
	collections::{HashMap, HashSet},
	convert::{TryFrom, TryInto},
};

impl TryFrom<RuntimeMetadataV9> for Metadata {
	type Error = Error;

	fn try_from(metadata: RuntimeMetadataV9) -> Result<Self, Self::Error> {
...
fn convert_module(index: usize, module: v9::ModuleMetadata) -> Result<ModuleMetadata, Error> {
...
fn convert_event(event: v9::EventMetadata) -> Result<ModuleEventMetadata, Error> {
...
fn convert_entry(prefix: String, entry: v9::StorageEntryMetadata) -> Result<StorageMetadata, Error> {
...
impl From<TempStorageHasher> for DesubStorageHasher {
	fn from(hasher: TempStorageHasher) -> DesubStorageHasher {
		...
		match hasher {
			...
			StorageHasher::Blake2_128Concat => DesubStorageHasher::Blake2_128Concat,
